from typing import List
from ai_quoting.schemas import ProviderOffer
from ai_quoting.adapters.base import Adapter

class Screwfix(Adapter):
    name = "screwfix"
    def fetch_offers(self, product_name: str, qty: float, uom: str) -> List[ProviderOffer]:
        if "cement" in product_name.lower():
            return [
                ProviderOffer(supplier="Screwfix", unit_price=8.20, pack_size_qty=25, pack_size_uom="kg", title="Blue Circle Cement 25kg", url="https://www.screwfix.ie/", option="delivery")
            ]
        return []
